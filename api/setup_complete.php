...
try {
    ...
    echo "4. Recriando tabela seat_history...\n";
    ...
    $result = $historyStmt->execute([$testSeatId, 'minor', 'Teste de inserção']);
    
    if ($result) {
        echo "✓ Inserção no histórico bem sucedida usando seatId: $testSeatId\n";
        $lastId = $pdo->lastInsertId();
        $check = $pdo->query("SELECT * FROM seat_history WHERE id = $lastId")->fetch(PDO::FETCH_ASSOC);
        echo "Dados inseridos:\n";
        print_r($check);
    } else {
        echo "✗ Falha na inserção no histórico\n";
    }
